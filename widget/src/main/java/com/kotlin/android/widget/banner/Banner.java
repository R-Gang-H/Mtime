...
public class Banner extends RelativeLayout {
    ...
    private class OnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
        ...
        public void onPageScrollStateChanged(int state) {
            if (state == ViewPager2.SCROLL_STATE_DRAGGING) {
                if (!isPolling) {
                    isBeginPagerChange = true;
                } else if (tempPosition == sidePage - 1) {
                    isBeginPagerChange = false;
    ...
    private class BannerAdapterWrapper extends RecyclerView.Adapter<ViewHolder> {
        ...
        public int getItemCount() {
            return getRealCount() > 1 && isPolling ? getRealCount() + needPage : getRealCount();
        }
    ...
    public Banner setPageMargin(int tlWidth, int brWidth, int pageMargin) {
        ...
        int start = tlWidth > 0 ? tlWidth + Math.abs(pageMargin) : 0;
        int end = brWidth + Math.abs(pageMargin);
        addPageTransformer(new MarginPageTransformer(pageMargin));
        RecyclerView recyclerView = (RecyclerView) viewPager2.getChildAt(0);
        if (viewPager2.getOrientation() == ViewPager2.ORIENTATION_VERTICAL) {
            recyclerView.setPadding(viewPager2.getPaddingLeft(), start, viewPager2.getPaddingRight(), end);
        } else {
            recyclerView.setPadding(start, viewPager2.getPaddingTop(), end, viewPager2.getPaddingBottom());
        }
    ...
    public Banner setOnPageChangeListener(ViewPager2.OnPageChangeCallback listener) {
     ...
     * @param polling
     */
    public Banner setPolling(boolean polling) {
        isPolling = polling;
        if (!polling) {
            setAutoPlay(false);
        }
