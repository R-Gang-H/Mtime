 ...
 * @author o.s
 */
class DropAdapter(val action: ((suit: Suit) -> Unit)? = null) : RecyclerView.Adapter<DropAdapter.ViewHolder>() {
    ...
    fun setData(data: ArrayList<Suit>?) {
        this.data.clear()
        data?.let {
            this.data.addAll(it)
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(TextView(parent.context).apply {
            ...
            getDrawableStateList(
                    normalRes = R.drawable.ic_drop_item_label_normal,
                    selectedRes = R.drawable.ic_drop_item_label_highlight
            ).apply {
