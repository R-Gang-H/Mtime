 ...
 */
class FindMovieRepository : BaseRepository() {
    /**
     * 获取banner数据
     */
...
//            apiMTime.getRegionPublishList(CommConstant.RCMD_REGION_HOME_BANNER)
            apiMTime.getRegionPublishList(CommConstant.RCMD_REGION_FIND_MOVIE_BANNER)
        }
    }
    ...
    suspend fun loadFilmList(
        ...
        return request {
            apiMTime.list(null, nextStamp, pageSize)
        }
    }
     ...
     * 返回筛选条件
     */
    suspend fun loadFilterCondition(): ApiResult<ConditionResult> {
        return request {
            apiMTime.getFindMovieCondition()
        }
    }
    ...
    @Query("page") page: Long = 0L,
    @Query("pageSize") pageSize: Long = 20L,
    @Query("year") year: Long = 0L,
    ...
    suspend fun loadFilmFilterResult(
        ...
        page: String = "",
        pageSize: Long = 0L,
        year: String = "-1:-1",//from:to
        genreTypes: Long = 0L,
        area: Long = 0L,
        sortType: Long = 0L,
    ): ApiResult<MovieSearchResult> {
        return request {
            apiMTime.loadFilmFilterResult(
                keyword,
                page = page,
                pageSize = pageSize,
                year = year,
        ...
        /*return ApiResult.Success(
            MovieSearchResult(
                ...
                (0..10).map {
                    Movie(
                        img = "http://img5.mtime.cn/mg/2020/07/02/150952.61926540.jpg",
                        movieType = "爱情/故事/惊悚",
                        name = "复仇者联盟",
                        rating = 9.0,
                        rYear = 2022,
                        content = "一股突如其来的强大邪恶势力对地球造成致命威胁，没有任何一个超级英雄能够单独抵挡。长期致力于保护全球安危的神 ..."
                    )
                }
            )
        )*/
