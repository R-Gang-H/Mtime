...
public class MovieScoreSeekBar extends View {
    ...
    public void setProgressDrawable(Drawable d) {
        if (mProgressDrawable != d) {
            ...
            doRefreshProgress(mScore, false, false);
        }
    }

    public void setScore(int score) {
        setScoreInner(score, false);
    }

    private void setScoreInner(int score, boolean fromUser) {
        score = constrain(score, mSegmentCount);
        if (score == mScore) {
            return;
        }
        mScore = score;
        doRefreshProgress(mScore, true, fromUser);
    }
    ...
    private void doRefreshProgress(int progress, boolean callBackToApp, boolean fromUser) {
        invalidate();
        if (callBackToApp) {
            onProgressRefresh(progress, fromUser);
        }
    }
    ...
    private void trackTouchEvent(MotionEvent event) {
        ...
        setScoreInner((int) Math.ceil(progress), true);
    }


    public interface OnSeekBarChangeListener {
        void onProgressChanged(MovieScoreSeekBar seekBar, int progress, boolean fromUser);
