...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int                       tabTextSelectedColor  = 0xFF0075C4;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabTextSelectedColor = a.getColor(R.styleable.PagerSlidingTabStrip_textSelectedColor, tabTextSelectedColor);
    ...
    private void addTab(final int position, View tab) {
        ...
        if (position == noPagerPosition) {
            if (noPagerTabClickListener != null) {
                tab.setOnClickListener(noPagerTabClickListener);
            }
        } else {
        ...
        //因为他喵的有广告的时候还不让显示tab
        if (tab instanceof TextView && ((TextView) tab).getText().toString().trim().equals("")) {
            tab.setPadding(0, 0, 0, 0);
            tabsContainer.addView(tab, position, defaultTabLayoutParams);
        }
        else {
            // TODO: 16/8/29 注释掉这里，因为它在的话发现页5个tab时字会显示不全。加这个干嘛的，有没有影响？ 
    ...
    private int noPagerPosition = -1;
    private View.OnClickListener noPagerTabClickListener;
    // 设置某个tab不能带动下面的viewpager滑动
    public void setTabHasNoPager(int position, View.OnClickListener clickListener){
        noPagerPosition = position;
        noPagerTabClickListener = clickListener;
    }
